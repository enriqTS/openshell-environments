...
function parseArgs(argv) {
  ...
  if (kind !== 'host') fail("usage: export-pi-host-package host --version VERSION --output DIR --compatibility FILE --asset-archive FILE [--source DIR]");
  ...
  if (!options.compatibility || !options.assetArchive) fail("host export requires --compatibility and --asset-archive");
  return options;
}
...
function hostMappings(compatibility) {
  return [
    { source: "clients/pi/host/pi", path: "bin/pi", mode: "0755" },
    { source: "clients/pi/host/pi", path: "bin/pi-openshell", mode: "0755" },
    { source: "clients/pi/host/pi-openshell-hook", path: "bin/pi-openshell-hook", mode: "0755" },
    { source: "clients/pi/host/lib/pi-openshell-settings.mjs", path: "lib/pi-openshell-settings.mjs", mode: "0755" },
    { source: "clients/pi/host/lib/pi-openshell-sessions.mjs", path: "lib/pi-openshell-sessions.mjs", mode: "0755" },
    { source: "clients/pi/host/lib/pi-openshell-provider", path: "lib/pi-openshell-provider", mode: "0755" },
    { source: "clients/pi/host/providers/pi-codex.yaml", path: "providers/pi-codex.yaml", mode: "0644" },
...
async function build(options) {
  ...
  const mappings = hostMappings(options.compatibility);
  ...
  try {
    ...
    const manifest = {
      ...
      source: { repository: "https://github.com/enriqTS/openshell-environments", revision }, files,
    };
    ...
    const archivePath = join(outputDir, `pi-openshell-${options.version}.tar.gz`);
...
build(parseArgs(process.argv.slice(2))).catch((error) => {
  console.error(`export-pi-host-package: ${error.message}`);
  process.exit(1);
});

...
Phase 5 complete. `ghcr.io/enriqts/openshell-environments/{base,pi}:0.2.0`, `pi-assets-v0.1.0`, and `pi-openshell-v0.1.0` are all published, signed/attested, and independently verified. The 0.1.0 whole-tree image build remains the rollback baseline, reachable at git tag `v0.1.0`.
...
Next action: Phase 6 (clean-machine acceptance on a second computer, and switching this machine's real `pi` over to the published install).

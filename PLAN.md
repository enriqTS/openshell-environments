...
- [x] Fixed the first host setup failure correctly: do not enable global Providers v2. Composition is unnecessary for endpoint-bound placeholders here, and enabling it rejects the active Pi sandbox because its provider host endpoints overlap the broad `allowed_ips` general-web endpoint with conflicting metadata. A trailing `--yes` remains harmlessly accepted for users retrying the prior command.
- [x] Restored `clients/claude/provider.yaml` after an accidental local truncation and added `--replace` to delete/recreate a provider configured with the wrong token.

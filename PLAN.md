...
Phase 4 CI is implemented and locally verified but not yet triggered; no image has been published to GHCR. The 0.1.0 whole-tree image build remains the rollback baseline, reachable at git tag `v0.1.0`; the environment release version moved to 0.2.0 for the new image composition.
...
Completed (Phase 4, publish OCI images — CI implemented, not yet triggered):
...
Next action: get explicit approval to push both repositories and their release tags, watch the real CI runs, then move to Phase 5 (host integration package: its own release/tag pattern in `pi-customizations`, an installer, `compatibility.json` generation, XDG install/rollback).

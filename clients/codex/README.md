...
The setup command reads `${CODEX_HOME:-$HOME/.codex}/auth.json` only on the host, imports `clients/codex/provider.yaml`, stores OAuth/refresh material through the gateway, enables gateway-managed refresh, and writes only the non-secret provider name to `${XDG_CONFIG_HOME:-$HOME/.config}/openshell-clients/codex.provider`. The image entrypoint creates each sandbox's `auth.json` from opaque gateway handles—not raw host tokens. Because invoking the launcher explicitly selects and uploads the current workspace, the entrypoint also marks exactly that canonical Git/workspace root as trusted in its ephemeral mode-0600 `config.toml`; it does not import host or project-local Codex configuration. Re-run setup after revoking or replacing the host login. `CODEX_OPENSHELL_PROVIDER=none` explicitly returns to disposable interactive login.

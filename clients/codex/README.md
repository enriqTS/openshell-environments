...
A minimum OpenShell client for running the [Codex](https://www.npmjs.com/package/@openai/codex) CLI in a sandbox: filesystem writes limited to the workdir and the `codex` user's home directory, general public web on ports 80/443 only (see `../../policies/base.yaml` and `../../docs/security-model.md`). Unlike `clients/pi/`, this is deliberately lightweight — no credential-provider hooks, no host-state synchronization, no published image. Log in interactively each time you use a new sandbox; nothing under `/home/codex` survives sandbox deletion. The launcher deliberately does not copy a host `OPENAI_API_KEY` into the sandbox.

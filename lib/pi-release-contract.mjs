...
const ASSET_PATH = /^(APPEND_SYSTEM\.md|agents\/[^/].*|extensions\/[^/].*|skills\/[^/].*|themes\/[^/].*)$/;
...
export function validateAssetManifest(manifest) {
  ...
  for (const [index, file] of (manifest.files ?? []).entries()) {
    if (object(file) && file.target !== "agent") errors.push(`files[${index}].target is invalid`);
  }
  return errors;
}
...
export function validateHostManifest(manifest) {
  ...
  if (!object(manifest.source)) errors.push("source is invalid");
    ...
    if (manifest.source.repository !== "https://github.com/enriqTS/openshell-environments") errors.push("source.repository is invalid");
    check(manifest.source.revision, REVISION, "source.revision", errors);
  }
